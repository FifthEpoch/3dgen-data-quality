...
# adding an extra option for custom captioning
@click.option('--caption_type', help='Use different types of captions from [pseudo_captions, human_captions, gpt4_captions]', type=str, default='pseudo_captions')
...
def train(
    ...
    **kwargs,
    ...
    assert caption_type in ['pseudo_captions', 'human_captions', 'gpt4_captions']

    assert gen_class in ['car', 'motorbike', 'chair', 'table', 'plane', 'all']

    caption_path = os.path.join('data/{caption_type}/', gen_class, 'id_captions.json')
    ...
    if rank == 0:
        with torch.no_grad():
            ...
            if gen_class == 'chair':
                fixed_text = [
                    'A mid-century wooden dining chair.',
                    'Bukly gray couch that looks heavy. ',
                    'A red Victorian chair without arm.',
                    'A gaming chair with high back and blue pattern.'
                ]
            if gen_class == 'car':
                fixed_text = [
                    'A yellow racing car.',
