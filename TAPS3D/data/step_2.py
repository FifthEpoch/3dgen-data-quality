...
class ImageDataset(data.Dataset):
    def __init__(self, image_root, image_transform):
        ...
        for root, dirs, files in os.walk(image_root):
            if files != []:
                files = [os.path.join(root, a) for a in files if re.match(r'^\d{2}\.png$', a)]
                self.img_list.append(files[np.random.RandomState(seed).randint(len(files))])
            for root_dir in dirs:
                for sub_root, sub_dirs, subdir_files in os.walk(root_dir):
                    if files != []:
                        files = [os.path.join(root, root_dir, a) for a in files if re.match(r'^\d{2}\.png$', a)]
                        self.img_list.append(files[np.random.RandomState(seed).randint(len(files))])
        ...
        self.img_list.sort()
        print(f"ImageDataset_init: {len(self.img_list)}")
    ...
    def __getitem__(self, index):
        ...
        img_index = self.img_list[index].split('/')[-3]
