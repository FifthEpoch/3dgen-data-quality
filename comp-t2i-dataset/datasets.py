...
class BaseDataset(data.Dataset):
    def __init__(self, data_dir, dataset_name, comp_type, split,
                 image_transform=None, tokenizer=None, category=None, caption_type=None):
        self.data_dir = data_dir
        self.dataset_name = dataset_name
        self.comp_type = comp_type # for ShapeNet, use comp_type for different types of caption
        self.split = split
        ...
        if dataset_name == 'custom':
                if split == 'test_unseen_diverse_style':
                    anno_data_path = os.path.join(data_dir, caption_type, category, comp_type, split, "data.pkl")
                else:
                    anno_data_path = os.path.join(data_dir, caption_type, category, comp_type, "data.pkl")
                with open(anno_data_path, "rb") as f:
                    self.anno_data = pickle.load(f)
                
                split_path = os.path.join(data_dir, caption_type, category, comp_type, "split.pkl")
                ...
                img_root = "/home/ptclient/text_guided_3D_gen/TAPS3D/save_inference_results"
                self.image_dir = os.path.join(img_root, self.caption_type, self.category, self.comp_type, self.split)
    ...
    def get_image(self, image_id, raw=False, preprocess=True):
        # HERE
        image_path = os.path.join(self.image_dir, image_id+"_0", "None_000000.png")
